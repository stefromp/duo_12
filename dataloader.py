...
def get_dataset(dataset_name,
  ...
  elif dataset_name == 'custom_local':
    # Load from local text files
    LOGGER.info(f'Loading custom local dataset from files')
    if mode == 'train' and custom_train_file:
      data_file = custom_train_file
    elif mode in ['validation', 'test'] and custom_valid_file:
  ...
  if dataset_name in ['lambada', 'openwebtext-train',
    ...
    # For custom_local, dataset is already properly structured
    data = dataset[mode]
  else:
    data = dataset[mode]
...
def get_dataloaders(config, tokenizer, skip_train=False,
  ...
  # Use config.trainer.devices instead of torch.cuda.device_count()
  # to respect the user's device configuration
